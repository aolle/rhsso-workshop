...
[#presetup]
== Configuring the Realm, roles and users
...
** Set `Valid Redirect URIs` as any root context of your application, for example, if your petclinic application is available at `http://domain.example.com/`, write `http://domain.example.com/*`.
** Click `Save`.

image::secapp/quarkus-petclinic-client-cfg.png[]

** Click `Credentials` tab and write down the `Secret` value, we will need it later.
...
* Set `User Resource Permission` as Name. `Default Resource` as `Resources`. Apply the `Default Policy`. Click `Save`.
