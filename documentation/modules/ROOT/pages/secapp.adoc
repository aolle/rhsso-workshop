= Securing your application with Red Hat Single Sign-On
...
* Click the `Role Mappings` tab. Add `assistant` and `vet` as `Assigned Roles`.

image::secapp/staff-roles.png[]

[#quarkus-app]
== Securing a Quarkus application using OpenID Connect (OIDC)
...
Let's configure our client athorization.
...
* Set `Default Group Policy` as the `Name`. Select `staff` from the `Groups` list, click `Select`. Be sure that the `Logic` is set to `Negative`. Finally, click `Save`.

image::secapp/group-policy.png[]

* You will be redirected back again to the `Add Resource Permission` page. Set `User Resource Permission` as `Name`. `Default Resource` as `Resources`. The recently created policy should have been added automatically. Click `Save`.
...
** Log in as `elisa`. The request will be redirected to the application after a sucessful login.
