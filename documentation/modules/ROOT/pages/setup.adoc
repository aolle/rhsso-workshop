...
Red Hat Single Sign-On can be deployed using the Operator or the application template.

The Operator automates and simplifies the administration tasks in Red Hat OpenShift using the custom resources (CRs). It means that we can create Red Hat Single Sign-On resources using these custom resources instead of using the Red Hat Single Sign-On admin console.
...
WARNING: If the Red Hat Single Sign-On fails during deployment, check if a `LimitRange` exists in the current Red Hat OpenShift project. Sometimes, the `RHPDS` environment creates a `LimitRange` automatically when a new project is created. Check also the health checks probes, the initial delay may require a higher value.

NOTE: For this workshop, we will assume that Red Hat Single Sign-On has been installed using the templates and imagestream method explained in xref:setup.adoc#deployrhsso-templates[1.2.2 Deploying Red Hat Single Sign-On for Red Hat OpenShift using the templates].
Additionally, for convenience and simplification purposes, the applications and services will be deployed in the same project where Red Hat Single Sign-On is deployed.

[#deployrhsso-operator]
=== Deploying Red Hat Single Sign-On for OpenShift using the Operator

You can use the Red Hat OpenShift Container Platform web console to subscribe and deploy the `Red Hat Single Sign-on Operator`. The Red Hat Single Sign-On Operator simplifies the deployment process and its ease to use, but less flexible than the template based deployment.
...
NOTE: Write down the auto generated `RH-SSO Administrator Username` and `RH-SSO Administrator Password` required for login into the Red Hat Single Sign-On admin console.
...
Later, we will see how we can integrate Red Hat Single Sign-On with IdM/FreeIPA.

Staying logged in into your Red Hat OpenShift platform, and for convenience, select the `sso` project that has been created previously. Follow those steps for deploying a FreeIPA instance on top of Red Hat OpenShift.
